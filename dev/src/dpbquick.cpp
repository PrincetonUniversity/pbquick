                  ...
                  int *np, int *nfac)
{
  ...
  jf = 0;
  maxf = nfac[m - kt];
  if(kt > 0) maxf = R::imax2(nfac[kt],maxf);
...
void fft_factor(int n, int *pmaxf, int *pmaxp)
{
  ...
  if (m_fac+kt > 15) {		/* error - too many factors */
    ...
    if (kt > 0) maxf = R::imax2(nfac[kt-1], maxf);
    if (kt > 1) maxf = R::imax2(nfac[kt-2], maxf);
    if (kt > 2) maxf = R::imax2(nfac[kt-3], maxf);
  }
  *pmaxf = maxf;
  *pmaxp = maxp;
			     ...
			     Rcpp::NumericVector& pp)
{
  ...
  std::vector<double> avec(n+1), bvec(n+1);
  ...
  std::vector<double> work(4*maxf);
  std::vector<int> iwork(maxp);
  
  fft_work(&(avec[0]), &(bvec[0]), 1, m, 1, -1, &(work[0]), &(iwork[0]));    
